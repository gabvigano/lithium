...
pub fn load_static_map(
    ...
    let map: Vec<components::StaticSpec> = ron::from_str(&data).expect("invalid static map format");
...
pub fn load_dynamic_map(
    ...
    let map: Vec<components::DynamicSpec> = ron::from_str(&data).expect("invalid dynamic map format");
    let mut entities = Vec::new();
    ...
    for obj in map {
        ...
        world.transform.insert(entity, obj.transform.into());
        world.rigid_body.insert(entity, obj.rigid_body.into());
        world.surface.insert(entity, obj.surface.into());
        world.shape.insert(entity, obj.shape);
        world.material.insert(entity, obj.material.into());
    }
